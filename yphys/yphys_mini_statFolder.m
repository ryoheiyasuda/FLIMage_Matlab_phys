function mini = yphys_mini_statFolder(yphys, ch)

lowRise = 0; %ms 
highRise = 4; %ms remove rise time slower than this.
lowDecay = 2; %ms
highDecay = 8; %ms 
thAmp = 5; %pA Threshold for Peak Amp.
thSse = 3.5; %Threshold for PeakAmp / SSE. Should be 3 - 4.
thsse_b = 3.5; %PeakAmp / sse_baseline; Baseline fluctuation.
...
%         end
    ...
    %%%%%%Fitting%%%%%%%%%%%%
        if ch == 1
            b_peak_pos = b.peak_pos;
        elseif ch == 2
            b_peak_pos = b.peak_pos2;
        end
        
        for k=1:length(b_peak_pos)
            mini_count = mini_count + 1;
            pos1 = b_peak_pos(k);
            if ch == 1
                offset1 = b.base_amp(k);
                %offset1 = mean(data1(pos1:pos1+nptb-1));
                scale1 = b.peak_amp(k);
            elseif ch == 2
                offset1 = b.base_amp2(k);
                %offset1 = mean(data1(pos1:pos1+nptb-1));
                scale1 = b.peak_amp2(k);
            end
            data2 = data1(pos1:pos1+npt-1);
            beta0 = [1, 8, scale1, offset1, -1];
            xdata1T = [1:npt]/yphys.inputRate*1000;
            data2 = data2(:)';
            try
                betahat = nlinfit(xdata1T, data2, @yphys_mini_fitting, beta0);
                fitted_temp2 = yphys_mini_fitting(betahat, xdata1T);
                tau1 = abs(betahat(1));
                tau2 = abs(betahat(2));
                if tau1 > tau2
                    tau2 = abs(betahat(1));
    ...
    disp(['Baseline (pA): ', num2str(mean(base_amp)), '+/-', num2str(std(base_amp))]);
