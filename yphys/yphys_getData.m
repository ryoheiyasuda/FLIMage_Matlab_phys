...
try
    ...
    %figure(200);
        gain = ones(1, state.yphys.init.n_physChannels);
        for ch = 1:state.yphys.init.n_physChannels
            if state.yphys.acq.cclamp(1)
                gain(ch) = state.yphys.acq.gainC(1);
            else
                gain(ch) = state.yphys.acq.gainV(1);
            end
        end
        ...
        if state.yphys.acq.phys_counter == 0
            filenames=dir(fullfile(state.files.savePath, filesep, yphys.saveDirName, filesep, 'yphys*.mat'));
        ...
        t = 1:length(data1(:,1));
        for ch = 1:state.yphys.init.n_physChannels
            dataA = [t(:)/rate*1000, data1(:, ch)/gain(ch)];
            evalc(['state.yphys.acq.data', num2str(ch), '= dataA']); 
        end
        state.yphys.acq.data = state.yphys.acq.data1;
        ...
        if isfield(state, 'files')
            ...
            filedir = [state.files.savePath, filesep, yphys.saveDirName];
           ...
           if get(gh.yphys.stimScope.saveCheck, 'Value')
                ...
                %pause(0.3);
                if exist([filedir, filesep, filen, '.mat'], 'file')
                    %disp('loading average ....');
                    yphys_loadYphys([filedir, filesep, filen, '.mat']);
                    yphys_averageData;
                end
           end
        end
