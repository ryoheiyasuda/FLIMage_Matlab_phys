...
% Last Modified by GUIDE v2.5 21-Nov-2015 12:07:10
...
% --- Executes on selection change in channel_pop.
function channel_pop_Callback(hObject, eventdata, handles)
% hObject    handle to channel_pop (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns channel_pop contents as cell array
%        contents{get(hObject,'Value')} returns selected item from channel_pop

global state
state.yphys.internal.activeCh = get(hObject, 'value');

% --- Executes during object creation, after setting all properties.
function channel_pop_CreateFcn(hObject, eventdata, handles)
% hObject    handle to channel_pop (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
set(hObject, 'value', state.yphys.internal.activeCh);
