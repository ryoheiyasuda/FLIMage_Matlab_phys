...
state.yphys.init.imaging_on = 0;
state.yhys.init.yphys_on = 1;

if ~isfield(state, 'acq')
    state.yphys.init.imaging_on = 0;
    state.init.triggerInputTerminal = 'PFI6';
    state.init.triggerBoardID = 'Dev2'; %
    state.init.triggerLineID = 0;
end

state.yphys.init.externalTrigger = 'PFI2';
yphys.saveDirName = 'spc';
...
if state.yphys.init.imaging_on
    ...
    state.files.savePath = 'C:\Users\yasudalab\Documents\data\';
end

%%%%
state.yphys.init.vclampBoardIndex = 'Dev2';
state.yphys.init.n_physChannels = 1;
state.yphys.init.n_stimChannels = 1;
state.yphys.init.vclampLineIndex = 1;
%%%%
state.yphys.init.phys_boardIndex = 'Dev2';
state.yphys.init.phys_patchChannelIndex = 0;
state.yphys.init.phys_stimChannelIndex = 1;
state.yphys.init.phys_patchChannelIndex2 = 2;
state.yphys.init.phys_stimChannelIndex2 = 3;
state.yphys.init.phys_dataIndex = 0; %Channel0
state.yphys.init.phys_dataIndex2 = 1; %Channel2
...
state.yphys.internal.activeCh = 1;
 state.yphys.acq.n_physChannels = state.yphys.init.n_physChannels;
...
yphys.data.data2 = [1:32, 1:2];
yphys.aveData = [1:32, 1:2];
yphys.aveData2 = [1:32, 1:2];
yphys.fwindow = 5;
%%%
%
...
% else
%     win64 = 1;
%     state.yhys.init.yphys_on = 0;
% end

if state.yphys.init.multiclamp == 1 && state.yhys.init.yphys_on
    if start_external
...
if ~board_active && restart_all
    ...
    if state.yhys.init.yphys_on == 1
            try
                state.yphys.init.phys_input = Task(['yphys input', num2str(round(rand(1)*10000))]);
                state.yphys.init.phys_input.createAIVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_dataIndex, 'data1', -10, 10);
                if (state.yphys.init.n_physChannels > 1)
                    state.yphys.init.phys_input.createAIVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_dataIndex2, 'data2', -10, 10);
                end
    ...
    if state.yhys.init.yphys_on
            ...
            state.yphys.init.phys_inputPatch.createAIVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_dataIndex, 'data patch1', -10, 10);
            if (state.yphys.init.n_physChannels > 1)
                state.yphys.init.phys_inputPatch.createAIVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_dataIndex2, 'data patch2', -10, 10);
            end            
            ...
            if (state.yphys.init.n_physChannels > 1)
                     state.yphys.init.phys_patch.createAOVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_patchChannelIndex2, 'patch2', -10, 10);
            end
        ...
        %
            ...
            if (state.yphys.init.n_physChannels > 1)
                state.yphys.init.phys_patch_Static.createAOVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_patchChannelIndex2, 'patch2 static', -10, 10);
            end
            ...
            state.yphys.init.phys_both.createAOVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_patchChannelIndex, 'patch both', -10, 10);
            if (state.yphys.init.n_physChannels > 1)
                state.yphys.init.phys_both.createAOVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_patchChannelIndex2, 'patch both2', -10, 10);
            end
            state.yphys.init.phys_both.createAOVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_stimChannelIndex, 'stim both', -10, 10);
            if (state.yphys.init.n_stimChannels > 1)
                state.yphys.init.phys_both.createAOVoltageChan(state.yphys.init.phys_boardIndex, state.yphys.init.phys_stimChannelIndex2, 'stim both2', -10, 10);
            end
    ...
    if state.yphys.init.imaging_on
        ...
        %     state.yphys.init.shutterAO.cfgSampClkTiming(state.yphys.acq.outputRate, 'DAQmx_Val_FiniteSamps');         
            ...
            try
                    delete(state.init.hTrigger);
            end
            state.init.hTrigger = Task(['Internal Start Trigger Source', num2str(round(rand(1)*10000))]);
