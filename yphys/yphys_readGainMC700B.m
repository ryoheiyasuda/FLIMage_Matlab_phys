...
if ~isempty(token)
    for i=1:state.yphys.init.n_physChannels
        a(i).ID = token{i}.ID;
        token2 = MultiClampTelegraph('getAmplifier', double(a(i).ID));
        a(i).Mode = token2.uOperatingMode;
        a(i).Primary_Gain = token2.dAlpha;
        a(i).ScaleFactor = token2.dScaleFactor;
        a(i).External_Cmd_Sens = token2.dExtCmdSens;
    end
end
