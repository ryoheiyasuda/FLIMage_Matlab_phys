...
minV = 0;
if tag == 3 || tag == 100 %strcmp(tag, 'uncage')||strcmp(tag, 'auto')
    if Dheight > 100
        Dheight = 100;
    end
end
...
%                 param = state.yphys.acq.pulse{3, addP(k)};
%             end
            param = state.yphys.acq.pulse{tag, addP(k)};
            rate2 = param.freq;
            nstim2 = param.nstim;
            dwell2 = param.dwell;
            ampc2 = param.amp;
...
if tag == 3 || tag == 100 %strcmpi(tag, 'uncage') || strcmpi(tag, 'auto')
    a(a == 0) = 1;
else
end
...
strA = sprintf('set(gh.yphys.pulsePlot%d, ''XData'', (1:length(a))/state.yphys.acq.outputRate*1000, ''YData'', a)', tag);
try
    evalc(strA);
catch
    disp(strA);
