...
% Last Modified by GUIDE v2.5 01-Dec-2015 14:09:29
...
axes(handles.axes4);
gh.yphys.pulsePlot4 = plot(zeros(32, 1));
ylabel(handles.axes4, 'Patch2 (mV/pA)');
set(handles.axes4, 'xTickLabel', []);

axes(handles.axes5);
gh.yphys.pulsePlot5 = plot(zeros(32, 1));
ylabel(handles.axes5, 'Stim2 (mV)');
...
if strcmp(get(handles.start, 'String'), 'Start')
    ...
    if ~isempty(cycleSet)&& get(gh.yphys.stimScope.loop_on, 'value')
        cyclePos = mod(state.yphys.acq.phys_counter-1, length(cycleSet))+1;
        cycleStr = num2str(cycleSet(cyclePos));
        state.yphys.internal.startP = state.yphys.acq.phys_counter;
...
mutual_exclude(hObject, handles);
 ...
 % --- Executes on button press in patch2radio.
function patch2radio_Callback(hObject, eventdata, handles)
% hObject    handle to patch2radio (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of patch2radio
mutual_exclude(hObject, handles);

% --- Executes on button press in stim2radio.
function stim2radio_Callback(hObject, eventdata, handles)
% hObject    handle to stim2radio (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of stim2radio
mutual_exclude(hObject, handles);
...
Radiobuttons = [handles.UncageRadio,handles.StimRadio, handles.PatchRadio, ...
                handles.stim2radio, handles.patch2radio];
off = (Radiobuttons == h);
...
if isfield(yphys, 'filename')
    ...
    filenamestr = sprintf('%s%03d', basename, num);
end
filepath = [pathstr, filesep, filenamestr, extstr];
if exist(filepath, 'file')
    yphys_loadYphys(filepath);
end
...
folder_name = uigetdir(state.files.savePath);
state.files.savePath = folder_name;
...
% --- Executes on button press in loop_on.
function loop_on_Callback(hObject, eventdata, handles)
% hObject    handle to loop_on (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of loop_on
