...
if notimer
    ...
    for ch = state.yphys.init.n_physChannels:-1:1
        if state.yphys.acq.cclamp(ch)
            ...
            a = [a; ones(phase(2), 1)];
            a = [a; zeros(phase(3), 1)];
            a = a*state.yphys.acq.camplitude/state.yphys.acq.commandSensC(ch);            
            ...
            a = [a; ones(phase(2), 1)];
            a = [a; zeros(phase(3), 1)];
            a = a*state.yphys.acq.vamplitude/state.yphys.acq.commandSensV(ch);
    ...
    if (state.yphys.init.n_physChannels == 1)
        state.yphys.acq.patchdata = a1{1};
    elseif (state.yphys.init.n_physChannels > 1)
        state.yphys.acq.patchdata = [];
        for ch = 1:state.yphys.init.n_physChannels
            if ch ~= state.yphys.internal.activeCh
                a1{ch} = a1{state.yphys.internal.activeCh}*0;                
            end 
            state.yphys.acq.patchdata = [state.yphys.acq.patchdata, a1{ch}];
        end
    else
    ...
    %try
        colorj = {'red', 'blue', 'green'};
        axes (gh.yphys.scope.trace);
        gh.yphys.patchPlot = plot(zeros(32,1), 'color', colorj{ch});
%         for ch = 1:state.yphys.init.n_physChannels
%             gh.yphys.patchPlot(ch) = plot(zeros(32,1), 'color', colorj{ch});
%             hold on;
%         end
%         hold off;
    %end
