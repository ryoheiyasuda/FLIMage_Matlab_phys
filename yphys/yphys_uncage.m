...
%%%%%%%%%%%%%%%%%%%
amp2 = 5;
pockelsOutput2 = yphys_mkPulse(rate, nstim, dwell, amp2, delay, sLength, addP, 3);
...
%     pockelsOutput2(pockelsOutput2_t > 0) = state.init.eom.lut(uncageP, ampc); 
    pockelsOutput2 = yphys_mkPulse(rate, nstim, dwell, ampc, delay, sLength, addP, 3);
...
if (ap || stim) 
    ...
    if state.yhys.init.yphys_on && (~page)
...
state.yphys.acq.uncagePercentPulses = yphys_mkPulse(rate, nstim, dwell, ampc, delay, sLength, addP,3);
...
if state.yhys.init.yphys_on && get(gh.yphys.stimScope.saveCheck, 'value') && (~page)
