...
[PATHSTR,fNAME,EXT] = fileparts(spc.filename);
if isempty(PATHSTR)
    PATHSTR = state.files.savePath;
end
spc.filename = [fNAME, EXT];

if strfind(fNAME, 'max')
    baseName = fNAME(1:end-7);
    fnA = fNAME(end-6:end-4);
elseif strfind(fNAME, 'glu')
    baseName = fNAME(1:end-7);
    fnA = fNAME(end-6:end-4);
else
    baseName = fNAME(1:end-3);
    fnA = fNAME(end-2:end);
end


if exist(PATHSTR) == 7
    graph_savePath = PATHSTR;
    spc_savePath = PATHSTR;
else
    graph_savePath = pwd;
    spc_savePath = pwd;
end
...
if exist([PATHSTR, filesep, baseName, '.mat'])
    load([PATHSTR, filesep, baseName, '.mat'], baseName);
    evalc(['a = ', baseName]);
    try
        str1 = a.textbox;
    catch
...
tnPhoton = sum(lifetime);
display(['total number of photon: ', num2str(tnPhoton)]);
display(['Predited noise (ns) ', num2str(a.tau_m2(fc) - a.tau_m2(fc)*2/sqrt(tnPhoton)), ' - ', num2str(a.tau_m2(fc) + a.tau_m2(fc)*2/sqrt(tnPhoton))]);
if ~isfield(a, 'time2')
    try
        a.time2 = a.time;
    end
...
if get(gh.motorControls.pmPosnID, 'Value')-1 >= 1
    a.position(length(a.fraction)) = get(gh.motorControls.pmPosnID, 'Value')-1;
    %spc.scanHeader.motor.position = get(gh.motorControls.pmPosnID, 'Value')-1;
else
    a.position(length(a.fraction)) = 1;
end


eval([baseName, '= a;']);
save([spc_savePath, filesep, baseName, '.mat'], baseName);
...
if isfield(state, 'acq')
	if 0
 		ydata_line1 = mean(spc.project, 2);
%         ydata_line2 = mean(state.acq.acquiredData{2}, 2);
         ydata_line1 = ydata_line1 / mean(ydata_line1(1:10)) - 1;
%         ydata_line2 = ydata_line2 - mean(ydata_line2(1:20));
        ydata_line3 = spc_calcLinescan;
        bas = ydata_line3(1:10);
        n1 = ~isnan(bas);
        bas(~n1) = 0;
        bas1 = sum(bas)/sum(n1);
        ydata_line3 = ydata_line3 / bas1 - 1;
        xdata_line = 1:length(ydata_line3);
		try
